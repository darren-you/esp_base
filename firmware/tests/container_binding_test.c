...
int main(void)
{
    ...
    assert(esp_base_container_with_firmware_set(&claim,
        ESP_BASE_OTA_FIRMWARE_CONFIRMED, inspect_set, &owner) == ECONTAINER_SLOTS_OK);
    ...
    assert(esp_base_container_with_firmware_set(&nested,
        ESP_BASE_OTA_FIRMWARE_CONFIRMED, inspect_set, &owner) == ECONTAINER_SLOTS_BUSY);
    ...
    assert(esp_base_container_with_firmware_set(&claim,
        ESP_BASE_OTA_FIRMWARE_CONFIRMED, inspect_set, &owner) == ECONTAINER_SLOTS_UNCERTAIN);
    ...
    assert(esp_base_container_with_firmware_set(&claim,
        ESP_BASE_OTA_FIRMWARE_CONFIRMED, inspect_set, &owner) == ECONTAINER_SLOTS_UNCERTAIN);
    assert(operation_calls == 3U && esp_base_storage_claim_active(&claim));
    ...
    observed_result = ESP_BASE_OTA_FIRMWARE_OK;
    assert(esp_base_container_with_firmware_set(&claim,
        ESP_BASE_OTA_FIRMWARE_CONFIRMED, uncertain_operation, &owner) ==
