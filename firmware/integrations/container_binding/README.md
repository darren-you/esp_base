...
配置包产品时，当前 `PENDING_VERIFY` 固件在启动检查后明确拒绝确认并请求 IDF 回滚；`ota.start` 在产品 configured 的 `EMPTY/RUNNING/BLOCKED` 状态下因启动 claim 持续被占用而返回 `BUSY`。这防止尚无联合 OTA 持久转换合同的产品产生孤立新固件。后续联合升级还需要：Base 在当前 A 已 VALID、目标 C 已签名写入但尚未选 boot 的窗口提供精确 `PREPARED_CANDIDATE` 固件集合；Container 持久化 package-write／reuse／no-package 的固件转换，再由 Base 选 boot；新 boot 使用精确 trial/health/confirm 状态，并处理回滚候选。现有 confirmed/pending 两种观察不能表达前置窗口，不得猜 otadata，也不得直接改 Container blob。
